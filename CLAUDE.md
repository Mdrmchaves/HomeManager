...
- **Angular Signals**: all new components use `signal()` for local state, `computed()` for derived values, and `toSignal()` to convert HTTP Observables into Signals. `ChangeDetectorRef` and `markForCheck()` are not used — signals update the template automatically, including inside `async/await` and native browser callbacks. To force a data reload, use a `BehaviorSubject` as a trigger combined with `toSignal()`. `ChangeDetectionStrategy.OnPush` is no longer needed on signal-based components.
