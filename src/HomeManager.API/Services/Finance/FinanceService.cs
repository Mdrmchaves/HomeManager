...
public class FinanceService : IFinanceService
{
    ...
    public async Task<ApiResponse<List<TemplateResponse>>> GetTemplatesAsync(Guid householdId, Guid userId)
    {
        try
        {
            if (!await HasAccessAsync(householdId, userId))
                return ApiResponse<List<TemplateResponse>>.ErrorResponse("Access denied");

            var templates = await m_context.FinanceTemplates
                .Include(t => t.Account)
                .Where(t => t.HouseholdId == householdId)
                .OrderBy(t => t.Description)
                .ToListAsync();

            return ApiResponse<List<TemplateResponse>>.SuccessResponse(
                templates.Select(TemplateResponse.FromEntity).ToList());
        }
        catch (Exception ex)
        {
            m_logger.LogError(ex, "Error getting templates for household {HouseholdId}", householdId);
            return ApiResponse<List<TemplateResponse>>.ErrorResponse($"Error: {ex.Message}");
        }
    }

    public async Task<ApiResponse<TemplateResponse>> CreateTemplateAsync(CreateTemplateRequest request, Guid userId)
    {
        try
        {
            if (!await HasAccessAsync(request.HouseholdId, userId))
                return ApiResponse<TemplateResponse>.ErrorResponse("Access denied");
            ...
            if (request.AccountId.HasValue)
            {
                account = await m_context.FinanceAccounts.FindAsync(request.AccountId.Value);
                if (account is null || account.HouseholdId != request.HouseholdId)
                    return ApiResponse<TemplateResponse>.ErrorResponse("Account not found");
            }

            var template = new FinanceTemplate
            {
                Id = Guid.NewGuid(),
                HouseholdId = request.HouseholdId,
                AccountId = request.AccountId,
            ...
            m_context.FinanceTemplates.Add(template);
            await m_context.SaveChangesAsync();

            template.Account = account;
            m_logger.LogInformation("Template {TemplateId} created for household {HouseholdId}", template.Id, request.HouseholdId);
            return ApiResponse<TemplateResponse>.SuccessResponse(TemplateResponse.FromEntity(template), "Template created");
        }
        catch (Exception ex)
        {
            m_logger.LogError(ex, "Error creating template for household {HouseholdId}", request.HouseholdId);
            return ApiResponse<TemplateResponse>.ErrorResponse($"Error: {ex.Message}");
        }
    }

    public async Task<ApiResponse<bool>> DeleteTemplateAsync(Guid id, Guid userId)
    {
        try
        {
            var template = await m_context.FinanceTemplates.FindAsync(id);
            if (template is null)
                return ApiResponse<bool>.ErrorResponse("Template not found");

            if (!await HasAccessAsync(template.HouseholdId, userId))
                return ApiResponse<bool>.ErrorResponse("Access denied");

            m_context.FinanceTemplates.Remove(template);
            await m_context.SaveChangesAsync();

            m_logger.LogInformation("Template {TemplateId} deleted", id);
            return ApiResponse<bool>.SuccessResponse(true, "Template deleted");
        }
        catch (Exception ex)
        {
            m_logger.LogError(ex, "Error deleting template {TemplateId}", id);
            return ApiResponse<bool>.ErrorResponse($"Error: {ex.Message}");
        }
    }

    public async Task<ApiResponse<ApplyResult>> ApplyTemplatesAsync(ApplyTemplatesRequest request, Guid userId)
    {
        try
        {
            if (!await HasAccessAsync(request.HouseholdId, userId))
                return ApiResponse<ApplyResult>.ErrorResponse("Access denied");

            if (!System.Text.RegularExpressions.Regex.IsMatch(request.Month, @"^\d{4}-\d{2}$"))
                return ApiResponse<ApplyResult>.ErrorResponse("Month must be in YYYY-MM format");
            ...
            // Find existing template-generated transactions for this month to avoid duplicates
            var existingTemplateIds = await m_context.FinanceTransactions
                .Where(tx => tx.HouseholdId == request.HouseholdId
                    && tx.RefMonth == request.Month
                    && tx.FromTemplateId.HasValue)
                .Select(tx => tx.FromTemplateId!.Value)
                .ToListAsync();
            ...
            var newTransactions = new List<FinanceTransaction>();
            ...
            foreach (var template in templates)
            {
                ...
                // Clamp day to last day of month (e.g. day=31 in February → day=28/29)
                var daysInMonth = DateTime.DaysInMonth(year, month);
                var day = Math.Min(template.DayOfMonth, daysInMonth);
                var date = new DateOnly(year, month, day);
                var refMonth = CalcRefMonth(date, template.Account);

                newTransactions.Add(new FinanceTransaction
                {
                    Id = Guid.NewGuid(),
                    HouseholdId = request.HouseholdId,
                    CreatedBy = userId,
                    ...
                    Date = date,
                    RefMonth = refMonth,
                    Type = string.IsNullOrEmpty(template.Category) ? "income" : "expense",
            ...
            m_context.FinanceTransactions.AddRange(newTransactions);
            await m_context.SaveChangesAsync();

            m_logger.LogInformation("Applied {Generated} templates for household {HouseholdId} month {Month}",
                generated, request.HouseholdId, request.Month);
            return ApiResponse<ApplyResult>.SuccessResponse(new ApplyResult(generated, skipped),
                $"Generated {generated} transactions, skipped {skipped} already applied");
        }
        catch (Exception ex)
        {
            m_logger.LogError(ex, "Error applying templates for household {HouseholdId}", request.HouseholdId);
            return ApiResponse<ApplyResult>.ErrorResponse($"Error: {ex.Message}");
        }
    }
